...
EX)
  ...
  INSERT 10개 완료! ----> 1개 업무처리공간에 10개의 INSERT 담김 ----> ROLLBACK ----> 모든 작업 없던 일로 처리

  1) COMMIT : 최종저장 완료!
              메모리 버퍼(트랜잭션)에 임시 저장된 데이터 변경 사항을 DB에 반영

  2) ROLLBACK : 최종저장 취소!
                메모리 버퍼(트랜잭션)에 임시 저장된 데이터 변경사항을 삭제하고 마지막 커밋 상태로 돌아감 (== 작업 내용 삭제)

  3) SAVEPOINT : 임시저장
                  메모리 버퍼(트랜잭션)에 저장 지점을 설정해서 ROLLBACK 수행 시 전체 작업을 삭제하는 것이 아닌
