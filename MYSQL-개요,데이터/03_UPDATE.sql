...
2. ENUM 자료형 이나 SET 자료형 과 같이 컬럼에 들어갈 수 있는 데이터가 정해져 있고, 정해진 데이터의 범위를 벗어났을 때 발생하는 에러
   EX) 컬럼값으로 SHOPPING, READING 과 같은 데이터만 들어갈 수 있도록 설정한 컬럼에 쇼핑, 책읽기와 같은 데이터를 넣으려 했을 때 발생
   ...
   SET SQL_SAFE_UPDATES = 1; -- 안전모드 실행
